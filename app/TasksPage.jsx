...
export default function TasksPage() {
	...
	const groups = [
		...
		{
			label: "Tomorrow",
			tasks: taskList.filter((t2) => t2.due === "Tomorrow"),
		},
		{
			label: "This Week",
			tasks: taskList.filter((t2) => !["Today", "Tomorrow"].includes(t2.due)),
		},
	];
	const filtered =
		filter === "All"
			...
			: groups.map((g) => ({
					...
					tasks: g.tasks.filter(
						(t2) =>
							t2.priority === filter.toLowerCase() ||
							t2.agent === filter ||
							t2.category === filter,
					),
				}));
		...
		<div>
			...
			>
				{[
					...
					{
						label: "Due Today",
						val: taskList.filter((x) => x.due === "Today").length,
						color: t.amber,
					},
				].map((m) => (
				...
				>
					...
					>
						...
						{[
							...
							["Assign To", AGENTS_DATA.map((a) => a.name)],
							[
								"Category",
								[
									"Follow-up",
							...
							["Due Date", ["Today", "Tomorrow", "Mar 14", "Mar 15", "Mar 16"]],
						].map(([label, opts]) => (
