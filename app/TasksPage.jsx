...
export default function TasksPage() {
	...
	const groups = [
		...
		{
			label: "This Week",
			items: taskList.filter((x) => !["Today", "Tomorrow"].includes(x.due)),
		},
	];
		...
		<div>
			...
			>
				...
				<KPI
					t={t}
					label="Done"
					value={taskList.filter((x) => x.done).length}
					sub="completed"
				/>
				<KPI
					t={t}
					label="Due Today"
					value={taskList.filter((x) => x.due === "Today").length}
					sub="need attention"
				/>
				<KPI
					t={t}
			...
			{groups.map((group) => {
				const show2 =
					filter === "All"
						? group.items
						: group.items.filter(
								(x) => x.priority === filter || x.category === filter,
							);
						...
						{show2.map((task) => {
							...
							return (
								<Card
									...
									t={t}
									p={14}
									style={{ marginBottom: 8, opacity: task.done ? 0.4 : 1 }}
								>
									<div
										style={{
											display: "flex",
									...
									>
										...
										<div style={{ flex: 1 }}>
											...
											<div style={{ display: "flex", gap: 6 }}>
												...
												{task.client !== "—" && <Tag t={t}>{task.client}</Tag>}
											</div>
										</div>
										<span
											style={{
			...
			<Modal show={show} t={t} title="New Task" onClose={() => setShow(false)}>
				...
				>
					<Field t={t} label="Category">
						<Select
							t={t}
							options={["Client Work", "Business Dev", "Finance", "Admin"]}
						/>
					</Field>
