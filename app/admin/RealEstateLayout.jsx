...
import { NAV_MAIN, NAV_BOTTOM, AGENTS_DATA, THEMES, TASKS_DATA } from "./data";
...
export default function RealEstateLayout({ children }) {
	...
	if (!mounted) {
		return (
			<CRMContext.Provider value={contextValue}>
				<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#09090b", color: "#f0efe8", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
					Loading…
				</div>
			</CRMContext.Provider>
		);
	...
	return (
		<CRMContext.Provider value={contextValue}>
		<div className="crm-app" style={{ fontFamily: "'Plus Jakarta Sans', 'Helvetica Neue', sans-serif", background: t.bg, color: t.text, minHeight: "100vh", height: "100vh", display: "flex", flexDirection: "row", overflow: "hidden", transition: "background 0.3s, color 0.3s" }} suppressHydrationWarning>
			<style>{`
			...
			>
				<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingLeft: 4, marginBottom: 26 }}>
					{!sidebarCollapsed && !isMobile && (
						<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
							<div style={{ width: 34, height: 34, borderRadius: 10, background: `linear-gradient(135deg,${t.accent} 0%,#8a6020 100%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>⌂</div>
							<div>
								<div style={{ fontSize: 15, fontWeight: 800, letterSpacing: -0.4, color: t.text, lineHeight: 1.2 }}>EstateOS</div>
								<div style={{ fontSize: 10, color: t.textMuted }}>Real Estate CRM</div>
							</div>
						</div>
					)}
					{(sidebarCollapsed || isMobile) && (
						<div style={{ width: 34, height: 34, borderRadius: 10, background: `linear-gradient(135deg,${t.accent},#8a6020)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, margin: isMobile ? "0 auto" : "0" }}>⌂</div>
					)}
					{!sidebarCollapsed && !isMobile && (
						<button onClick={() => setSidebarCollapsed(true)} style={{ width: 24, height: 24, borderRadius: 6, background: t.surfaceB, border: `1px solid ${t.border}`, cursor: "pointer", color: t.textSub, fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
							‹
						</button>
					)}
				</div>
				{sidebarCollapsed && !isMobile && (
					<button onClick={() => setSidebarCollapsed(false)} style={{ width: 32, height: 32, borderRadius: 8, background: t.surfaceB, border: `1px solid ${t.border}`, cursor: "pointer", color: t.textSub, fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px" }}>
						›
					</button>
				)}
				...
				{(!sidebarCollapsed || isMobile) && (
					<>
						<div style={{ fontSize: 10, fontWeight: 700, color: t.textMuted, letterSpacing: 1.2, paddingLeft: 6, marginBottom: 8 }}>MAIN</div>
						{NAV_MAIN.map((item) => (
							<div
								key={item.id}
								...
								style={{ justifyContent: sidebarCollapsed && !isMobile ? "center" : "flex-start", padding: sidebarCollapsed && !isMobile ? "10px" : "9px 12px" }}
							>
						...
						<div style={{ fontSize: 10, fontWeight: 700, color: t.textMuted, letterSpacing: 1.2, paddingLeft: 6, marginTop: 20, marginBottom: 8 }}>AGENTS</div>
						{AGENTS_DATA.map((a) => (
							<div key={a.code} className="crm-ni" style={{ gap: 8 }}>
								<Avatar code={a.code} color={a.code === "SR" ? "#c8a96e" : a.code === "JM" ? "#60a5fa" : a.code === "KL" ? "#4ade80" : "#a78bfa"} size={26} />
								<div style={{ flex: 1, minWidth: 0 }}>
									...
									<div style={{ fontSize: 10, color: t.textMuted }}>{a.volume}</div>
								</div>
							</div>
						))}
					</>
				...
				<div style={{ marginTop: "auto" }}>
					{(!sidebarCollapsed || isMobile) && (
						<div style={{ background: t.surfaceAlt, borderRadius: 11, padding: "12px 14px", marginBottom: 10 }}>
							<div style={{ fontSize: 11, color: t.accent, fontWeight: 700, marginBottom: 4 }}>Q1 Target</div>
							<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 6 }}>
								<span style={{ fontSize: 20, fontWeight: 800, color: t.text }}>51%</span>
								<span style={{ fontSize: 11, color: t.textSub }}>$14.2M / $28M</span>
							</div>
							<div style={{ height: 5, borderRadius: 3, background: t.surfaceB }}>
								<div style={{ height: "100%", width: "51%", borderRadius: 3, background: `linear-gradient(90deg,${t.accent},#a07830)` }} />
							</div>
						</div>
					)}
					{NAV_BOTTOM.map((item) => (
			...
			<div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
				...
				>
					...
					<div style={{ display: "flex", alignItems: "center", flex: 1, minWidth: 0 }}>
						...
						<span style={{ color: t.textMuted, margin: "0 6px", fontSize: 12 }}>›</span>
						<span style={{ fontSize: 12, fontWeight: 600, color: t.text, textTransform: "capitalize" }}>{activePage}</span>
					</div>
					<div style={{ position: "relative" }}>
						<span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", fontSize: 13, color: t.textMuted }}>🔍</span>
						<input className="crm-srch" placeholder="Search leads, listings..." />
					</div>
					<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
						<div style={{ position: "relative" }}>
							<div className="crm-tbtn" onClick={() => setNotifOpen(!notifOpen)}>
								🔔
							</div>
							<div style={{ position: "absolute", top: 6, right: 6, width: 7, height: 7, borderRadius: "50%", background: t.red, border: `2px solid ${t.surface}` }} />
							{notifOpen && (
								<div style={{ position: "absolute", right: 0, top: 42, width: 300, maxWidth: "90vw", background: t.surface, border: `1px solid ${t.border}`, borderRadius: 14, padding: 16, zIndex: 200, boxShadow: t.shadowLg }}>
									<div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Notifications</div>
									{[
										{ text: "Dev Patel offer expires in 2 days", time: "Now", dot: t.red },
										{ text: "Marcus Webb — no follow-up in 5d", time: "1h ago", dot: t.amber },
										{ text: "2847 Lakeview showing confirmed", time: "2h ago", dot: t.green },
									].map((n, i) => (
						...
						<div style={{ width: 34, height: 34, borderRadius: 9, background: t.accentDim, color: t.accent, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, cursor: "pointer", border: `1px solid ${t.border}` }}>SR</div>
					</div>
				</header>

				<main style={{ flex: 1, overflow: "auto", padding: "26px 28px", background: t.bgSubtle }} onClick={() => notifOpen && setNotifOpen(false)}>
					<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
						<div>
							<h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: -0.5, color: t.text }}>{title}</h1>
							<p style={{ fontSize: 13, color: t.textSub, marginTop: 3 }}>{sub}</p>
						</div>
						<div style={{ display: "flex", gap: 8 }}>
							{activePage === "dashboard" && (
								<>
									<button style={{ padding: "9px 16px", borderRadius: 9, background: t.surfaceAlt, border: `1px solid ${t.border}`, color: t.textSub, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Export CSV</button>
									<button style={{ padding: "9px 18px", borderRadius: 9, background: t.accent, color: t.bg, border: "none", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>+ New Lead</button>
								</>
							)}
							{activePage === "reports" && <button style={{ padding: "9px 16px", borderRadius: 9, background: t.accent, color: t.bg, border: "none", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Export PDF</button>}
						</div>
					</div>
