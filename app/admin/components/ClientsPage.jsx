...
import { Card, Pill, Tag, Btn, CDot, Modal, Field, Input, Select } from "./Shared";
		...
		<div style={{ display: "flex", gap: 16 }}>
			<div style={{ flex: 1, minWidth: 0 }}>
				<div style={{ display: "flex", gap: 8, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
					{["All", ...Object.keys(STAGE_M)].map((s) => (
						<div
							...
							style={{
								...
								fontWeight: 700,
								cursor: "pointer",
								background: sf === s ? (STAGE_M[s]?.color || t.accent) : t.surfaceAlt,
					...
					<Btn t={t} primary onClick={() => setShow(true)} style={{ marginLeft: "auto" }}>
						+ New Client
					</Btn>
				</div>
							...
							>
								<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
									...
									<div>
										...
										<div style={{ fontSize: 11, color: t.textSub }}>{c.contact}</div>
									</div>
								</div>
								<Tag t={t}>{c.type}</Tag>
								...
								<span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 13, fontWeight: 700 }}>${c.value.toLocaleString()}</span>
								<span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 13, color: c.outstanding > 0 ? t.amber : t.textMuted }}>
									{c.outstanding > 0 ? `$${c.outstanding.toLocaleString()}` : "—"}
								</span>
								<span style={{ fontSize: 11, color: c.lastActivity === "Today" ? t.green : t.textMuted }}>{c.lastActivity}</span>
							</div>
						);
					})}
				</Card>
				...
				<div style={{ width: 280, flexShrink: 0 }}>
					<Card t={t} p={18} style={{ position: "sticky", top: 0 }}>
						<div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
							<span style={{ fontSize: 13, fontWeight: 700, color: t.text }}>Client Detail</span>
							<button onClick={() => setSel(null)} style={{ background: "none", border: "none", color: t.textSub, cursor: "pointer", fontSize: 16 }}>
								×
							</button>
						</div>
						<div style={{ textAlign: "center", padding: "16px 0", borderBottom: `1px solid ${t.border}`, marginBottom: 14 }}>
							<div
								style={{
									width: 48,
									height: 48,
							...
							<div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 4 }}>{sel.name}</div>
							<div style={{ fontSize: 12, color: t.textSub }}>{sel.contact}</div>
						</div>
						{[["Email", sel.email], ["Phone", sel.phone], ["Type", sel.type], ["Started", sel.started], ["Projects", sel.projects], ["Outstanding", sel.outstanding > 0 ? `$${sel.outstanding.toLocaleString()}` : "None"]].map(([k, v]) => (
						...
						<div style={{ marginTop: 12, marginBottom: 14 }}>
							<div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, letterSpacing: 0.6, marginBottom: 6 }}>TAGS</div>
			...
			{show && (
				<Modal t={t} title="New Client" onClose={() => setShow(false)}>
