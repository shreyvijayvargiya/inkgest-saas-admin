...
export default function TimeLogPage() {
	...
	const billH = TIME_LOGS.filter((l) => l.billable).reduce(
		(s, l) => s + l.hours,
		0,
	);
	const nonH = TIME_LOGS.filter((l) => !l.billable).reduce(
		(s, l) => s + l.hours,
		0,
	);
	const billVal = TIME_LOGS.filter((l) => l.billable).reduce(
		...
		<div>
			...
			>
				...
				<KPI
					...
					value={`$${billVal.toLocaleString()}`}
					sub="at avg rate"
				/>
				<KPI
					t={t}
					label="Utilization"
					value={`${Math.round((billH / (billH + nonH)) * 100)}%`}
					sub="target: 80%"
				/>
			</div>
					...
					{logs.map((log) => (
						<Card key={log.id} t={t} p={14} style={{ marginBottom: 6 }}>
							<div
								style={{
									display: "grid",
									gridTemplateColumns: "2fr 1.5fr 0.8fr 0.8fr 0.8fr",
									alignItems: "center",
									gap: 12,
								}}
							>
								...
								<Pill
									color={log.billable ? t.green : t.textSub}
									dim={log.billable ? t.greenDim : t.surfaceAlt}
									small
								>
								...
								<span
									style={{
										fontFamily: "'Geist Mono',monospace",
										fontSize: 12,
										color: log.billable ? t.accent : t.textMuted,
									}}
								>
			...
			<Modal show={show} t={t} title="Log Time" onClose={() => setShow(false)}>
