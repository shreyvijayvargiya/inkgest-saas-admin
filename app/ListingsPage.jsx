...
export default function ListingsPage() {
	...
	const filtered = LISTINGS_DATA.filter(
		(l) => statusF === "All" || l.status === statusF,
	);
		...
		return (
			<div>
				...
				<div
					...
					className="crm-listing-detail-grid"
				>
					<div>
						<div
							style={{
						...
						<Card t={t} style={{ marginBottom: 14 }}>
							...
							>
								...
								<Pill
									color={statusMeta[selected.status]?.color || t.green}
									dim={
										statusMeta[selected.status]?.dim ||
										statusMeta["Active"]?.dim
									}
								>
									{selected.status}
								</Pill>
						...
						<Card t={t}>
							...
							>
								{[
									[
										"Garage",
										selected.garage > 0 ? `${selected.garage} car` : "N/A",
									],
									["Lot Size", selected.lot],
									["Year Built", selected.year],
									[
										"Agent",
										AGENTS_DATA.find((a) => a.code === selected.agent)?.name,
									],
									["Days on Market", `${selected.dom} days`],
									[
										"Price/sqft",
										`$${Math.round(selected.price / selected.sqft)}`,
									],
								].map(([k, v]) => (
									<div
										key={k}
					...
					<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
						...
						<Card t={t}>
							...
							{SHOWINGS.filter((s) => s.address === selected.address).length ===
								0 && (
								<div
									style={{
										fontSize: 13,
						...
						return (
							...
							>
								...
								<div style={{ padding: 16 }}>
									...
									<div
										style={{ fontSize: 12, color: t.textSub, marginBottom: 12 }}
									>
										{l.city} · {l.type}
									</div>
									<div
									...
									>
										<span
											style={{ fontSize: 20, fontWeight: 800, color: t.accent }}
										>
						...
						return (
							...
							>
								...
								<span style={{ fontSize: 12, color: t.textSub }}>
									{l.beds > 0 ? `${l.beds}/${l.baths}` : `—/${l.baths}`}
								</span>
								<Pill color={sm.color} dim={sm.dim} small>
									{l.status}
								</Pill>
								<span
									style={{
										fontSize: 12,
										color: l.dom > 20 ? t.red : t.textSub,
									}}
								>
									{l.dom}d
								</span>
								<div style={{ display: "flex", alignItems: "center", gap: 6 }}>
									...
									/>
									<span style={{ fontSize: 12, color: t.textSub }}>
										{AGENTS_DATA.find((a) => a.code === l.agent)?.name}
									</span>
								</div>
							</div>
						);
